...
const ImagePreviews = ({ images }: ImagePreviewsProps) => {
  ...
  return (
    /* mobile: 225px (half), sm and up: 450px */
    <div className="relative h-[225px] sm:h-[450px] w-full overflow-hidden">
      {images.map((image, index) => (
        ...
        >
          <Image
            ...
            fill
            priority={index === 0}
            // <-- tiny increase on small devices only
            className="object-cover cursor-pointer transition-transform duration-500 ease-in-out scale-105 sm:scale-100"
          />
        </div>
      ))}
